(function (w) {
  ...
  const burgerMenu = {
    init(rootSel, p = {}) {
      ...
      Core.qsa(cfg.closeBtnSel).forEach(btn => btn.addEventListener('click', close));
  ...
  // ===== submenu (FIXED: never close while :hover is true on trigger or submenu) =====
  ...
  const submenu = {
    init(rootSel, p = {}) {
      ...
      const scan = () => {
        ...
        const isHoveringAny = () => {
          // guard: if anything throws, assume hovering false
          try {
        ...
        const scheduleClose = () => {
          ...
          state.closeTimer = setTimeout(() => {
            // ключ: не закрываем, если курсор всё ещё на триггере/подменю
            if (isHoveringAny()) {
              scheduleClose(); // перепроверим позже
              return;
            }
            closeAll();
          }, cfg.closeDelay);
        ...
        pairs.forEach(({ t, pWrap, pInner, i }) => {
          ...
          // cancel close anywhere on overlay or inner
          ...
          // click logic (desktop click-through, mobile 2-tap)
          const onClickT = (e) => {
