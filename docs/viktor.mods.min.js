(function (w) {
  ...
  const stickyHeader = {
    init(rootSel, p = {}) {
      ...
      Core.qsa(rootSel || 'body').forEach(root => {
        ...
        stickyMap.set(root, { onScroll, header, cfg });
      });
    },
    destroy(rootSel) {
      Core.qsa(rootSel || 'body').forEach(root => {
  ...
  // ===== submenu (desktop: hover opens + click goes; mobile: click toggles) =====
  ...
  const submenu = {
    init(rootSel, p = {}) {
      const cfg = Object.assign({
        ...
        panelSel: '.uc-submenu',  // отдельные блоки ниже
        openClass: 'is-open',
      ...
      Core.qsa(rootSel || 'body').forEach(root => {
        ...
        pairs.forEach(({ t, p, i }) => {
          ...
          const onClick = (e) => {
            // Десктоп + mixed: клик должен вести на страницу
            if (cfg.mode === 'mixed' && isDesktop() && cfg.mixedClickThrough) return;

            // Мобилка (или click-mode): клик открывает/закрывает
            e.preventDefault();
            const isOpen = p.classList.contains(cfg.openClass);
            isOpen ? closeAll() : openOnly(i);
          };
  ...
  MyMods.destroy = (rootSel) => {
    ...
    console.log('[MyMods] destroy ok');
  };

  w.MyMods = MyMods;
  console.log('[MyMods] library loaded');
})(window);
