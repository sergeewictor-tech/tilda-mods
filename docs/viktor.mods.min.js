(function (w) {
  ...
  const Tilda = {};
  // throttled mount: меньше спама и меньше "дребезга" при мутациях
  ...
  // ===== Global hard reset (prevents "opened on load") =====
  const resetUIState = () => {
    const html = document.documentElement;
    html.classList.remove('is-menu-open');
    Core.unlockScroll();

    Core.qsa('.uc-menu.is-open').forEach(m => m.classList.remove('is-open'));
    Core.qsa('.uc-submenu.is-open').forEach(s => s.classList.remove('is-open'));
  ...
  const stickyHeader = {
    init(rootSel, p = {}) {
      ...
      Core.qsa(rootSel || 'body').forEach(root => {
        ...
        const hideTarget = Core.qs(cfg.hideTargetSel, header) || header;
        ...
        stickyMap.set(header, { onScroll, onResize, header, cfg, hideTarget });
      });
    }
  };
  ...
  const burgerMenu = {
    init(rootSel, p = {}) {
      ...
      Core.qsa(rootSel || 'body').forEach(root => {
        ...
        const closeBtns = Core.qsa(cfg.closeBtnSel);
        ...
        // hard reset на случай "залипших" классов
        ...
        const open = (e) => {
          ...
          // закрыть любые подменю перед открытием меню
          Core.qsa('.uc-submenu.is-open').forEach(s => s.classList.remove('is-open'));
          Core.qsa('.uc-header .submenu.is-active').forEach(t => {
            t.classList.remove('is-active');
            t.setAttribute('aria-expanded', 'false');
        ...
        burgerMap.set(menu, { open, close, onMenuClick, onLink, onKey });
      });
    }
  };
  ...
  const submenu = {
    init(rootSel, p = {}) {
      ...
      const header = Core.qs(cfg.headerSel);
      if (!header) return;

      // state хранится на самом header → можно пересканировать, если добавишь ещё подменю
      ...
      const scan = () => {
        ...
        pairs.forEach(({ t, pWrap, pInner, i }) => {
          ...
          const openOnly = (idx) => {
            // если открыт бургер — закрываем принудительно
            const html = document.documentElement;
          ...
          const scheduleClose = () => {
            ...
            state.closeTimer = setTimeout(closeAll, cfg.closeDelay);
          };
          const cancelClose = () => {
            clearTimeout(state.closeTimer);
          ...
          // сохраняем методы в state (нужны doc listeners)
          state.closeAll = closeAll;
        });

        // hard close on scan to prevent "stuck overlay" after DOM rearrange
        if (state.closeAll) state.closeAll();
      };
