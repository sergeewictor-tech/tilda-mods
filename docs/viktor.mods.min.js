  ...
  const burgerMenu = {
    init(rootSel, p = {}) {
      const cfg = Object.assign({
        ...
        // было false; но для "виджет открывается наверху" нужен фикс через freeze body
        lockScroll: false
      }, p);
      ...
      // ===== FIX: freeze page scroll while menu is open (solves widget opening at top) =====
      let savedScrollY = 0;
      let isFrozen = false;
      ...
      const freezePage = () => {
        ...
        // защита от скачка из-за исчезновения скроллбара
        Core.lockScroll();

        // "заморозка" документа без визуального прыжка
      ...
      const open = (e) => {
        ...
        // если кто-то включит cfg.lockScroll отдельно — не ломаем
      ...
      // FIX: panel может быть shape, поэтому ловим клики на всём меню в capture
