(function (w) {
  ...
  // ===== burgerMenu (FIXED: close on backdrop/outside panel + works from any scroll) =====
  ...
  const burgerMenu = {
    init(rootSel, p = {}) {
      ...
      Core.qsa(rootSel || 'body').forEach(root => {
        ...
        // FIX: закрываем при клике по любому месту, которое НЕ внутри panel
        const onMenuClick = (e) => {
          if (!cfg.closeOnBackdropClick || !isOpen) return;
          if (panel && panel.contains(e.target)) return; // клик внутри панели — не закрываем
          // клик в любом другом месте оверлея = фон
          close();
        };
        ...
        // если у тебя есть backdrop, он обычно перекрывает всё — но мы слушаем на menu, это надёжнее
        menu.addEventListener('click', onMenuClick);

        // клик по ссылкам внутри панели закрывает меню
        (panel || menu).addEventListener('click', onLink);

        document.addEventListener('keydown', onKey);

        burgerMap.set(root, { cfg, open, close, onMenuClick, onLink, onKey, menu, openBtn, closeBtn, backdrop, panel });
      });
    },
    destroy(rootSel) {
      Core.qsa(rootSel || 'body').forEach(root => {
        ...
        s.menu.removeEventListener('click', s.onMenuClick);
