  ...
  const burgerMenu = {
    init(rootSel, p = {}) {
      const cfg = Object.assign({
        ...
        // как у тебя было: отдельный lockScroll не используем
        lockScroll: false
      }, p);
      ...
      const freezePage = () => {
        ...
        savedScrollY = w.scrollY || document.documentElement.scrollTop || 0;
      ...
      const unfreezePage = () => {
        ...
        Core.unlockScroll();
        w.scrollTo(0, savedScrollY);
      };

      // ===== popup pin for .mods-pop when it opens offscreen =====
      const pinned = new Map(); // el -> { parent, next, style:{...} }
      let popMO = null;
      ...
      const rectOffscreen = (r) => {
        ...
        return (r.bottom < 0 || r.top > vh || r.right < 0 || r.left > vw || r.top < 0 || r.bottom > vh);
      };

      const applyPinnedStyle = (el) => {
        // через setProperty(..., 'important') чтобы переживать inline-стили виджета
        ...
        // z-index оставляем высокий, но гарантируем поверх меню
        let z = 0;
      ...
      const pinPopup = (el) => {
        if (!el || pinned.has(el)) {
          if (el) applyPinnedStyle(el);
          return;
        }

        // сохраняем DOM и inline-стили
        pinned.set(el, {
        ...
        // выносим в body, чтобы не зависеть от transform-контейнеров
      ...
      const handlePopups = Core.throttle(() => {
        if (!isOpen) return;

        // основной источник: все попапы виджетов у тебя обозначены как .mods-pop (wicca-pop точно)
        const pops = Core.qsa('.mods-pop', document);

        pops.forEach(el => {
          const vis = isVisible(el);
          if (!vis) {
            if (pinned.has(el)) unpinPopup(el);
            return;
          }
          const r = el.getBoundingClientRect();
          if (rectOffscreen(r)) pinPopup(el);
          else {
            // если раньше пинили, но теперь он стал в норме — НЕ трогаем (иначе может дергаться)
            // но если хочется откатить — раскомментируй:
            // if (pinned.has(el)) unpinPopup(el);
          }
        });
      }, 80);
      ...
      const startPopupObserver = () => {
        if (popMO) return;
        popMO = new MutationObserver(() => handlePopups());
        popMO.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['class','style'] });
      };

      const stopPopupObserver = () => {
        if (!popMO) return;
        try { popMO.disconnect(); } catch {}
        popMO = null;
      };

      const cleanupPinned = () => {
        // откатываем всё, что мы пинали
        Array.from(pinned.keys()).forEach(el => unpinPopup(el));
      };
      ...
      const open = (e) => {
        killEvent(e);                 // только для открытия
        if (isOpen) return;
      ...
      const close = (e, { kill = false } = {}) => {
        // ВАЖНО: по бекдропу НЕ глушим событие (иначе поповеры не получают outside click)
        if (kill) killEvent(e);
      ...
      const isBackdropTarget = (target) => {
        ...
        // закрываем только если кликнули по фону оверлея (carrier/filter) или по самому t-rec меню
      ...
      const onMenuClick = (e) => {
        ...
        // если клик по кнопкам — не считаем бекдропом
        if (e.target.closest(cfg.openBtnSel) || e.target.closest(cfg.closeBtnSel)) return;

        // если клик внутри panel (если она найдена) — не закрываем
        if (panel && panel.contains(e.target)) return;

        // закрываем ТОЛЬКО если это фон (иначе виджеты/элементы вне menu-panel не будут ломать меню)
        if (!isBackdropTarget(e.target)) return;

        close(); // без killEvent → поповеры смогут закрыться сами
      };
      ...
      const onLink = (e) => {
        ...
        // закрываем без event, чтобы не убить переход
        close();
      };

      // после кликов по виджетам — проверяем попапы (луна/праздники)
      const onAnyClickInMenu = (e) => {
        if (!isOpen) return;
        if (tooSoonAfterOpen()) return;
        if (e.target.closest(cfg.linkSel)) return;
        handlePopups();
      };
