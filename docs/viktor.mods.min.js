(function (w) {
  ...
  const burgerMenu = {
    init(rootSel, p = {}) {
      ...
      // ===== freeze page while menu open (твоя рабочая часть) =====
      let savedScrollY = 0;
      let isFrozen = false;
      ...
      // ===== NEW: keep widget popups visible while burger is open =====
      const pinnedOverlays = new Set();
      ...
      const isOffscreen = (el) => {
        ...
        // сильно выше/ниже/в сторону — точно вне экрана
        if (r.bottom < 0 || r.top > vh) return true;
        if (r.right < 0 || r.left > vw) return true;
        // или частично обрезан — тоже считаем проблемой
        if (r.top < 0 || r.bottom > vh) return true;
        if (r.left < 0 || r.right > vw) return true;
        return false;
      };

      const pinOverlayToViewport = (el) => {
        if (!el || pinnedOverlays.has(el)) return;
        // НЕ трогаем то, что реально находится внутри бургер-меню (пусть скроллится внутри меню)
        if (el.closest && el.closest('.uc-menu')) return;

        // пинаем только если реально вылезло за экран
        if (!isOffscreen(el)) return;

        // сохраняем, что меняли
        pinnedOverlays.add(el);

        // фиксируем в viewport, чтобы не зависеть от scroll/body-top
        el.dataset.modsPinnedByBurger = '1';
      ...
      const unpinAllOverlays = () => {
        pinnedOverlays.forEach(el => {
          if (!el) return;
          if (el.dataset) delete el.dataset.modsPinnedByBurger;
          // откатываем только то, что мы точно трогали: проще — чистим инлайны
          el.style.position = '';
          el.style.top = '';
          el.style.left = '';
          el.style.right = '';
      ...
      const fixOverlaysNow = () => {
        if (!isOpen) return;
        Core.qsa(overlaySelectors, document).forEach(pinOverlayToViewport);
      };

      let stopOverlayObserver = null;
      const startOverlayObserver = () => {
        if (stopOverlayObserver) return;
        stopOverlayObserver = Core.observe(() => {
          // даём DOM дорисоваться
          setTimeout(fixOverlaysNow, 0);
        }, document.body);
      };
      const stopOverlayObserverFn = () => {
        if (stopOverlayObserver) {
          stopOverlayObserver();
          stopOverlayObserver = null;
        }
      };
      ...
      const open = (e) => {
        ...
        // FIX: чтобы виджеты не улетали "вверх страницы"
        freezePage();

        // NEW: следим за появлением попапов/оверлеев от виджетов
      ...
      const close = (e) => {
        ...
        // NEW: откатываем всё, что пинали
        stopOverlayObserverFn();
        unpinAllOverlays();

        // FIX: возвращаем страницу как была
        unfreezePage();
      ...
      const onLink = (e) => {
        ...
        // FIX: закрываем без event, чтобы не убить переход по ссылке
        close();
      };

      // NEW: после любого клика внутри меню пробуем поправить оверлеи (если виджет открылся)
      const onAnyClickInMenu = (e) => {
        if (!isOpen) return;
        if (e.target.closest(cfg.openBtnSel) || e.target.closest(cfg.closeBtnSel)) return;
        // ссылки пусть живут своей жизнью
      ...
      menu.addEventListener('click', onLink, true);      // FIX: links capture
      menu.addEventListener('click', onAnyClickInMenu, true); // NEW
