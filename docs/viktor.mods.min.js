(function (w) {
  ...
  const burgerMenu = {
    init(rootSel, p = {}) {
      ...
      // ===== freeze page while menu open (как у тебя сейчас) =====
      let savedScrollY = 0;
      let isFrozen = false;
      ...
      // ===== NEW: pin offscreen popups (fix for "holidays opens at page top") =====
      const pinned = new Map(); // el -> prev inline styles

      const EXCLUDE_SEL = '.uc-menu, .uc-header, .uc-submenu, .header-shell';
      const KEY_RE = /(popup|popover|tooltip|dropdown|calendar|datepicker|modal|holiday|prazdn|event)/i;
      ...
      const rectOffscreen = (r) => {
        ...
        // если хоть часть ушла за экран — считаем проблемой
      ...
      const looksLikeOverlay = (el, cs) => {
        if (!el || el.nodeType !== 1) return false;
        if (el.closest && el.closest(EXCLUDE_SEL)) return false;
        ...
        const hasKey = KEY_RE.test(el.className || '') || KEY_RE.test(el.id || '');
        const highZ = !isNaN(zi) && zi >= 999;

        // либо явно попап по имени, либо высокий z-index
        if (!hasKey && !highZ) return false;

        return rectOffscreen(r);
      };

      const pinToViewport = (el) => {
        if (!el || pinned.has(el)) return;
      ...
      const scanAndPin = () => {
        ...
        for (let i = 0; i < all.length; i++) {
          const el = all[i];
          // пропускаем то, что уже в меню/хедере и т.п. (быстро)
          if (el.closest && el.closest(EXCLUDE_SEL)) continue;

          // быстрый фильтр по ключевым словам в class/id, чтобы не считать стили тысячам узлов
          const cls = (el.className || '');
          const id  = (el.id || '');
          const maybe = KEY_RE.test(cls) || KEY_RE.test(id);
          if (!maybe) continue;

          let cs;
          try { cs = getComputedStyle(el); } catch { continue; }
          if (looksLikeOverlay(el, cs)) pinToViewport(el);
        }
      };

      const scheduleScan = () => {
        // несколько тиков — на случай анимации/рендера
        setTimeout(scanAndPin, 0);
        setTimeout(scanAndPin, 60);
        setTimeout(scanAndPin, 180);
      };
      ...
      const open = (e) => {
        ...
        // сразу после открытия тоже проверим (на случай, если попап уже существует и просто показывается)
        scheduleScan();
      ...
      const onLink = (e) => {
        ...
        // закрываем без event, чтобы не убить переход по ссылке
        close();
      };

      // после кликов в меню (в т.ч. по виджетам) — подстраховка скана
      const onAnyClickInMenu = (e) => {
        if (!isOpen) return;
        if (tooSoonAfterOpen()) return;
        if (e.target.closest(cfg.openBtnSel) || e.target.closest(cfg.closeBtnSel)) return;
        if (e.target.closest(cfg.linkSel)) return; // ссылки не трогаем
        scheduleScan();
      };

      // capture=true чтобы перехватить раньше тильды
      ...
      menu.addEventListener('click', onAnyClickInMenu, true);// NEW: scan after widget clicks
